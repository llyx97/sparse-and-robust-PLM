...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        # Set the loss function for robust training
        if self.args.anneal_bias_range is not None:
            min_theta, max_theta = self.args.anneal_bias_range.split('_')
            min_theta, max_theta = float(min_theta), float(max_theta)
            is_anneal = True
        else:
            is_anneal = False

        if self.args.robust_training == 'reweighting':
            if is_anneal:
                self.loss_fn = ReweightAnnealed(total_steps=t_total, max_theta=max_theta, min_theta=min_theta)
            else:
                self.loss_fn = ReweightBaseline()
        elif self.args.robust_training == 'poe':
            if is_anneal:
                self.loss_fn = BiasProductAnnealed(total_steps=t_total, max_theta=max_theta, min_theta=min_theta)
            else:
                self.loss_fn = BiasProductBaseline()
        elif self.args.robust_training == 'regularization':
            if is_anneal:
                self.loss_fn = SmoothedDistillLossAnnealed(total_steps=t_total, max_theta=max_theta, min_theta=min_theta)
            else:
                self.loss_fn = SmoothedDistillLoss()
        elif self.args.robust_training is None:
            self.loss_fn = None
        ...
        if model_path is not None:
            ...
            try:
                ...
                self.global_step = json.load(open(os.path.join(model_path, 'save_step.json'), 'r'))['step']
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.args.global_grad_clip:
                        ...
                        for group in optimizer.param_groups:
                            for p in group['params']:
                                torch.nn.utils.clip_grad_norm_(p, self.args.max_grad_norm)
                    ...
                    if (self.args.logging_steps > 0 and self.global_step % self.args.logging_steps == 0) or (
                        ...
                        if self.model_args.structured:
                            ...
                            _, target_sparsity, _ = self.masker.masker_scheduler.step(cur_epoch=self.epoch)
                            mean_thresh = self.reset_threshold(model, target_sparsity)
                            zero_rate = self.save_model_mask(is_save=False)
                            self._log({'zero_rate': zero_rate})
                            self._log({'mean_thresh': mean_thresh})
                            # just to check whether my calculation of sparsity is the same as the masker_scheduler
                            sparsity = self.masker.masker_scheduler.get_sparsity_over_whole_model(self.model, self.masker)
                            self._log({'sparsity': sparsity})
                        ...
                        if self.args.anneal_bias_range is not None:
                            logs['bias_anneal'] = self.loss_fn.get_current_theta()
                        logging_loss = tr_loss
                        ...
                        if self.args.evaluate_during_training:
                            ...
                            if self.ood_datasets is not None:
                                for ood_dataset in self.ood_datasets:
                                    results.update(self.evaluate_ood(ood_dataset)[0])

                            result_to_json_file = os.path.join(self.args.output_dir, "result.json")
                            if os.path.isfile(result_to_json_file):
                                result_to_json = json.load(open(result_to_json_file, 'r'))
                                result_to_json.append(results)
                            else:
                                result_to_json = [results]
                            json.dump(result_to_json, open(result_to_json_file, 'w'))

                            for key, value in results.items():
                                #if key=='eval_acc' or key=='eval_mcc' or key=='eval_pearson':
                                if key==self.args.best_metric:
                                    if hasattr(self.model, 'classifiers') and int(key.split('_')[-1]) < len(self.model.classifiers)-1:
                                        continue
                                    if best_score<value and self.epoch>=self.args.num_train_epochs*self.args.start_step_ratio:                     # only save the consider the checkpoints after max_train_epoch*start_step_ratio
                                        best_score = value
                                        ...
                                        if hasattr(model, 'classifier') and self.model_args.train_classifier:
                    ...
                    if self.args.save_steps > 0 and self.global_step % self.args.save_steps == 0:
                        ...
                        # Save the model in case of continue training
                        ...
                        if hasattr(model, 'classifier') and self.model_args.train_classifier:
                            torch.save(model.classifier, output_dir+'/classifier.bin')
                        ...
                        json.dump({"step": self.global_step}, open(os.path.join(output_dir, "save_step.json"), 'w'))
    ...
    def _training_step(
        ...
        outputs = model(**inputs, loss_fn=self.loss_fn)
    ...
    def save_model_mask(self, output_dir: Optional[str] = None, is_save=True):
        mask_dict = {}
