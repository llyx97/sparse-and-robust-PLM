...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, loss_fn: ClfDistillLossFunction = None):
        super().__init__(config)
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                if loss_fn is not None:
                    loss = loss_fn.forward(pooled_output, logits.view(-1, self.num_labels), bias, teacher_probs, labels.view(-1))
                else:
